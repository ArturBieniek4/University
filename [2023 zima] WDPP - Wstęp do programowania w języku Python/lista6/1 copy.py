...
def sprobujwstawic(matryca, dowstawienia, pozycja_x, pozycja_y, kolor):
    ...
    for y in range(5):
        for x in range(5):
            ...
            if pozycja_y+y>0 and pozycja_x+x>0 and matryca[pozycja_y+y-1][pozycja_x+x-1]==kolor: return False
            if pozycja_y+y<4 and pozycja_x+x>0 and matryca[pozycja_y+y+1][pozycja_x+x-1]==kolor: return False
            if pozycja_y+y>0 and pozycja_x+x<4 and matryca[pozycja_y+y-1][pozycja_x+x+1]==kolor: return False
            if pozycja_y+y<4 and pozycja_x+x<4 and matryca[pozycja_y+y+1][pozycja_x+x+1]==kolor: return False
            if dowstawienia[y][x]=='#':
                if matryca[pozycja_y+y][pozycja_x+x]!=[1,1,1]: return False
                if x<4 and y<4 and dowstawienia[y+1][x+1]=='#' and (matryca[pozycja_y+y][pozycja_x+x+1]!=[1,1,1] or matryca[pozycja_y+y+1][pozycja_x+x]!=[1,1,1]): return False
