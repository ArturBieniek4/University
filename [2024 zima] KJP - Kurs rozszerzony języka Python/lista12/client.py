...
parser.add_argument("--addfriend", action='store_true',
...
parser.add_argument("--borrowbook", nargs=2,
                    metavar=('BOOK_ID', 'FRIEND_ID'), help="Borrow a book")
...
parser.add_argument("--deletebook", metavar="BOOK_ID",
                    type=int, help="Delete a book by ID")
parser.add_argument("--deletefriend", metavar="FRIEND_ID",
                    type=int, help="Delete a friend by ID")

if __name__ == "__main__":
    args = parser.parse_args()

    if args.addbook:
        res = requests.put("http://127.0.0.1:5000/book/add",
                           params={
                               "title": args.title,
                               "author": args.author,
                               "year": int(args.year)})
        ...
        res = requests.post(
            "http://127.0.0.1:5000/book/borrow",
            params={
                "book_id": book_id,
                "friend_id": friend_id})
        if res.json():
        ...
        res = requests.post("http://127.0.0.1:5000/book/return",
        ...
        if args.name and args.email:
            res = requests.put("http://127.0.0.1:5000/friend/add",
                               params={"name": args.name, "email": args.email})
            if res.json():
        ...
        res = requests.delete("http://127.0.0.1:5000/book/delete",
                              params={"book_id": args.deletebook})
        if res.json():
        ...
        res = requests.delete("http://127.0.0.1:5000/friend/delete",
                              params={"friend_id": args.deletefriend})
        if res.json():
