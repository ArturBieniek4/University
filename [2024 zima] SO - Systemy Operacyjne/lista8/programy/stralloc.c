...
static void *alloc_block(int8_t *data, uint8_t len) {
  ...
  while (!END_OF_CHUNK(data)) {
    if (BLOCK_USED(data)) {
      ...
      /* DONE: free block of exact size */
      result = CURRENT_PTR(data);
      BLOCK_HEADER(data) *=-1;
      break;
    } else if (BLOCK_SIZE(data) > len) {
      /* DONE: free block is too large */
      result = CURRENT_PTR(data);
      uint8_t left = len - BLOCK_SIZE(data);
      BLOCK_HEADER(data) = -left;
      break;
    } else if (!NEXT_BLOCK_FREE(data)) {
      /* DONE: next block is occupied or does not exists */
      MOVE_NEXT(data);
    } else if (NEXT_BLOCK_SIZE(data) <= len - BLOCK_SIZE(data)) {
      /* DONE: merge two free blocks, but do not allocate */
      BLOCK_HEADER(data) -= NEXT_BLOCK_SIZE(data);
    } else {
      /* DONE: merge two free blocks and allocate with split */
      result = CURRENT_PTR(data);
      uint8_t left = len - BLOCK_SIZE(data) - NEXT_BLOCK_SIZE(data);
      BLOCK_HEADER(data) = len; // Zaznacz jako zajęty
      NEXT_BLOCK_HEADER(data) = -left;
      break;
    }
  }
  return result;
...
#endif
#endif
  /* DONE: mark block as free */
  *(sstr-1) *= -1; // Zmień znak nagłówka na ujemny (blok jest wolny)
}
