...
static void db_open(db_t *db, const char *name, size_t size) {
  ...
  /* Inform OS about expected random access */
...
static bool db_rehash(db_t *db, size_t new_size) {
  ...
  for (size_t i = 0; i < db->size; i++) {
    if (!db_maybe_insert(new, db->entry[i])) {
      ...
      /* DONE: Remove new database, since rehashing failed. */
      db_close(new);
      Unlink(name);
      return false;
    }
  ...
  /* DONE Replace old database with new one, remove old database. */
  Munmap(db->entry, db->size * sizeof(entry_t));
  Unlink(db->name);
...
static void doit(const char *path, op_t mode) {
  ...
  /* Use fstat to check if stdin is a regular file or a pipe. */
  struct stat sb;
  Fstat(STDIN_FILENO, &sb);

  if (S_ISREG(sb.st_mode)) {
    /* If input is a regular file, use mmap for efficient reading. */
    char *input = Mmap(NULL, sb.st_size, PROT_READ|PROT_WRITE, MAP_PRIVATE, STDIN_FILENO, 0);

    /* Process each line. */
    char *line = input;
    char *end = input + sb.st_size;
    while (line < end) {
      line = consume_line(line, &db, mode);
    }

    /* Unmap the memory-mapped file. */
    Munmap(input, sb.st_size);
  } else {
    /* If input is a pipe or terminal, fallback to line-by-line reading. */
