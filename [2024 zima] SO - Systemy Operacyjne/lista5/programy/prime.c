...
static noreturn void filter_chain(pipe_t in) {
  ...
  if (!ReadNum(in, &prime)) {
    exit(EXIT_SUCCESS);  // Brak liczb do wczytania - zakończ proces
  }
  ...
  if (Fork() == 0) {  // child
    CloseWriteEnd(next_pipe);  // Zamknij zapis w nowej rurze w bieżącym procesie
    CloseReadEnd(in);          // Zamknij czytanie w poprzedniej rurze
    filter_chain(next_pipe);    // Uruchom nowy proces filtrujący
  } else {  //   parent
    CloseReadEnd(next_pipe);  // Zamknij czytanie w nowej rurze w bieżącym procesie
    filter(in, next_pipe, prime);
  }
  CloseWriteEnd(next_pipe);  // Zamknij zapis w nowej rurze po przetworzeniu wszystkich liczb
  Wait(NULL); // pochowaj dziecko
  exit(EXIT_SUCCESS);
}
